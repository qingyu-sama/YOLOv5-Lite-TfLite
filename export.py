...
def export_onnx(model, img, dynamic, output_names=None):
    try:
        ...
        torch.onnx.export(model, img, f, verbose=False, opset_version=12, input_names=['images'],
                          output_names=[
                              'classes', 'boxes'] if y is None else output_names,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='model_zoo/v5lite-e.pt',
                        help='weights path')  # from yolov5/models/
    parser.add_argument('--img-size', nargs='+', type=int,
                        default=[320, 320], help='image size')  # height, width
    parser.add_argument('--concat', action='store_true', help='concat or not')
    parser.add_argument('--mnnd', action='store_true',
                        help='mnn decode or not')
    parser.add_argument('--mnne', action='store_true',
                        help='mnn end2end or not')
    ...
    parser.add_argument('--end2end', action='store_true',
                        help='export the nms part in ONNX model')
